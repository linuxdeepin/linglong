...
utils::error::Result<void> removeOstreeRef(OstreeRepo *repo, const char *ref) noexcept
{
    ...
    ostree_repo_set_ref_immediate(repo, nullptr, ref, nullptr, nullptr, &gErr);
    if (gErr != nullptr) {
        return LINGLONG_ERR("ostree_repo_set_ref_immediate", gErr);
    }

    ostree_repo_prune(repo,
                                           ...
                                           const char *refspec) noexcept
{
    ...
    g_autoptr(GError) gErr = nullptr;
    ostree_repo_prepare_transaction(repo, NULL, NULL, &gErr);
    if (gErr != nullptr) {
    ...
    ostree_repo_write_directory_to_mtree(repo, dir, mtree, modifier, nullptr, &gErr);
    if (gErr != nullptr) {
        return LINGLONG_ERR("ostree_repo_write_directory_to_mtree", gErr);
    }

    g_autoptr(GFile) file = nullptr;
    ostree_repo_write_mtree(repo, mtree, &file, nullptr, &gErr);
    if (gErr != nullptr) {
        return LINGLONG_ERR("ostree_repo_write_mtree", gErr);
    }

    g_autofree char *commit = nullptr;
    ostree_repo_write_commit(repo,
                             nullptr,
                             nullptr,
                             nullptr,
                             nullptr,
    ...
    ostree_repo_commit_transaction(repo, NULL, NULL, &gErr);
    if (gErr != nullptr) {
                                                  ...
                                                  const char *refspec) noexcept
{
    ...
    layerDir.mkpath(".");
    Q_ASSERT(layerDir.exists());

    g_autoptr(GError) gErr = nullptr;
    ostree_repo_checkout_at(repo,
                            nullptr,
                            AT_FDCWD,
                            layerDir.absolutePath().toUtf8(),
                            refspec,
                            NULL,
                            &gErr);
    if (gErr != nullptr) {
                                                    ...
                                                    const QString &parent = "") noexcept
{
    ...
    ostree_repo_create(ostreeRepo, OSTREE_REPO_MODE_BARE_USER_ONLY, nullptr, &gErr);
    if (gErr != nullptr) {
...
{
    return this->ostreeRepoDir().absoluteFilePath("layers/" + ostreeSpecFromReference(ref, devel));
}

QDir OSTreeRepo::ostreeRepoDir() const noexcept
{
    Q_ASSERT(!this->repoDir.path().isEmpty());
    return this->repoDir.absoluteFilePath("repo");
}
    ...
    , apiClient(client)
{
...

    {
        ...
        ostreeRepo = ostree_repo_new(repoPath);
        ostree_repo_open(ostreeRepo, nullptr, &gErr);
        if (gErr == nullptr) {
    ...
    auto result = createOstreeRepo(this->repoDir.absolutePath(),
...
utils::error::Result<void> OSTreeRepo::remove(const package::Reference &ref, bool devel) noexcept
{
    ...
    auto result =
      removeOstreeRef(this->ostreeRepo.get(), ostreeSpecFromReference(ref, devel).toUtf8());
    if (!result) {
        return LINGLONG_ERR(result);
    }
                      ...
                      bool devel) noexcept
{
    ...
    ostree_repo_pull_with_options(this->ostreeRepo.get(),
                                  ...
                                  &gErr);
    if (gErr != nullptr) {
        auto err = LINGLONG_ERRV("ostree_repo_pull_with_options", gErr);
        taskContext->updateStatus(service::InstallTask::Failed, err.message());
        return;
    }
    ...
    auto result = handleRepositoryUpdate(this->ostreeRepo.get(),
                                         ...
                                         refString);
    if (!result) {
        taskContext->updateStatus(service::InstallTask::Failed, result.error().message());
        return;
    }
