                             ...
                             bool devel) noexcept
{
    ...
    if (!layerDir) {
        taskContext->updateStatus(InstallTask::Failed, layerDir.error().message());
        return;
    }

    auto info = layerDir->info();
    if (!info) {
        taskContext->updateStatus(InstallTask::Failed, info.error().message());
        return;
    }
    ...
    if (info->runtime) {
        ...
        t.addRollBack([this, &runtime, devel]() noexcept {
            auto result = this->repo.remove(*runtime, devel);
            if (!result) {
                qCritical() << result.error();
                Q_ASSERT(false);
            }
    ...
    if (!base) {
        taskContext->updateStatus(InstallTask::Failed, base.error().message());
        return;
    }
                            ...
                            bool devel) noexcept
{
    ...
    taskContext->updateStatus(InstallTask::preInstall, "prepare updating " + ref.toString());
    ...
    this->repo.pull(taskContext, newRef, devel);
    if (taskContext->currentStatus() == InstallTask::Failed
        || taskContext->currentStatus() == InstallTask::Canceled) {
        return;
    }
    ...
    auto layerDir = this->repo.getLayerDir(newRef);
    if (!layerDir) {
        taskContext->updateStatus(InstallTask::Failed, layerDir.error().message());
        return;
    }
