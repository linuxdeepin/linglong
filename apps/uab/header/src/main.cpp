...
std::string calculateDigest(int fd, std::size_t bundleOffset, std::size_t bundleLength) noexcept
{
    ...
    for (auto i = 0U; i < digestLength; i++) {
        stream << std::setw(2) << md_value.at(i);
    }
                               ...
                               const std::vector<std::string_view> &loaderArgs) noexcept
{
    ...
    for (const auto &layer : meta.layers) {
        ...
        if (kind == "app") {
            ...
            std::string_view baseStr = layer.info.base;
            const auto *splitSlash = std::find(baseStr.cbegin(), baseStr.cend(), '/');
            const auto *splitColon = std::find(baseStr.cbegin(), baseStr.cend(), ':');
            baseID = baseStr.substr(std::distance(baseStr.cbegin(), splitColon) + 1,
                                    splitSlash - splitColon);

            if (layer.info.runtime) {
                std::string_view runtimeStr = layer.info.runtime.value();
                const auto *splitSlash = std::find(runtimeStr.cbegin(), runtimeStr.cend(), '/');
                const auto *splitColon = std::find(runtimeStr.cbegin(), runtimeStr.cend(), ':');
